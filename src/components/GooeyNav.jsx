...
const GooeyNav = ({
  ...
  const colors = ["#9333ea", "#ec4899", "#a855f7", "#fbbf24"];
  ...
  // Synchronize with parent's initialActiveIndex
  useEffect(() => {
    ...
    <>
      <style>
        {`
          :root {
            ...
            ${isDark ? "--text-color: white; --text-color-shadow: 0 1px 1px rgba(0, 0, 0, 0.2);" : "--text-color: #1f2937; --text-color-shadow: none;"}
          }
          .gooey-effect {
            position: absolute;
            opacity: 1;
      ...
      <div className="relative" ref={containerRef}>
        ...
        >
          <ul
            ref={navRef}
            className="flex gap-2 lg:gap-4 list-none p-0 px-0 m-0 relative z-[3]"
            style={{
              color: isDark ? "white" : "#1f2937",
              textShadow: isDark ? "0 1px 1px rgba(0, 0, 0, 0.2)" : "none",
            }}
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`gooey-nav-li rounded-xl relative cursor-pointer transition-colors duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] text-white text-sm flex items-center gap-2 py-2 px-3 ${
